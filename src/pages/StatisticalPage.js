...
const StatisticalPage = () => {
    ...
    const handlePreviewReport = (e) => {
        ...
        if (status === 1) {
            ...
            {
                url_report = axiosApiInstance.defaults.baseURL + `/api/report/revenue_byOrder?${e.target.elements.ddfrom.value}&${e.target.elements.ddto.value}`;
                console.log(e.target.elements.ddfrom.value,e.target.elements.ddto.value)
            }
            else
                toast.error("Ngày bắt đầu phải nhỏ hơn hoặc bằng ngày kết thúc")
        ...
        if (url_report !== null) {
            ...
            if (tokens) {
                const token = JSON.parse(tokens).data.accessToken;
                const xhr = new XMLHttpRequest();
                xhr.open('GET', url_report, true);
                xhr.setRequestHeader('Authorization', `Bearer ${token}`);
                xhr.responseType = 'arraybuffer';
            ...
            console.error('url_report is null.');
        }
